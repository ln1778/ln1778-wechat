 ...
 */
 ...
 const subscribeMsgReq=(scope,templateId,reserved)=>{
  new Promise((r,j)=>{
    WeChat.subscribeMsgReq(scope,templateId,reserved,(err,result)=>{
      r(result);
    });
  });
};
...
const openCustomerServiceChat=(corpId,url)=>{
  return new Promise((r,j)=>{
   WeChat.openCustomerServiceChat(corpId,url,(err,result)=>{
         r(result);
       });
  });
};
...
export default {openWXApp,registerApp,isWXAppInstalled,once,WechatError,pay,shareToFavorite,shareToSession,shareToTimeline,
  sendAuthRequest,openCustomerServiceChat,getApiVersion,subscribeMsgReq,launchMiniProgramReq,
